...
FROM  (SELECT /*+ ordered use_hash(b) use_nl(d) */
	...
	-- disk I/O
      ...
      -- routine waits
       ...
       AND    nvl(b.time, 0) < i.kslestim 
       AND    d.indx = i.indx 
       AND    d.kslednam NOT IN  (select name from v$event_name where wait_class= 'Idle')
       AND    d.kslednam NOT IN  (select event from STATS$IDLE_EVENT)
