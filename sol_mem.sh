...
echo "" > $ShadUse
for i in $ShadPrc;do
   $Pmap $i | grep "read/write" | grep -v shared | awk '{ print $2 }' | awk -FK '{ print $1 }' >> $ShadUse
done
...
$Pmap $BkgdPrc | grep "shared" | awk '{ print $2 }' | awk -FK '{ print $1 }' >> $SharUse
...
if [ $ParmFound = "0" ];then
   ...
   $Pmap $2 | grep "read/write" | grep -v shared | awk '{ print $2 }' | awk -FK '{ print $1 }' >> $PidPUse
